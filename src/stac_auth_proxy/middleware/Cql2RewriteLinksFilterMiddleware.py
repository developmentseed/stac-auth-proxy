    ...
    async def _process_and_send_response(
        ...
        for link in links if isinstance(links, list) else []:
            ...
            if user_filter is None:
                link_body.pop("filter", None)
                link_body.pop("filter-lang", None)
            elif link_body.get("filter-lang") == "cql2-text":
                link_body["filter"] = user_filter.to_text()
            else:
                link_body["filter"] = user_filter.to_json()
